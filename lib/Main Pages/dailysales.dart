...
import 'package:flutter_application_1/Main%20Pages/AddSales.dart';

class MyDailySales extends StatefulWidget {
  const MyDailySales({Key? key}) : super(key: key);

  @override
  _MyDailySales createState() => _MyDailySales();
}

class _MyDailySales extends State<MyDailySales> {
  List data = [];
  List data1 = [];
  final sqlLite = CheckData();
  Future fetSalesData() async {
    var response = await sqlLite.fetSalesData();
    setState(() {
      data = response;
      // print(data);
    });
  ...
  Future fetchDash(id) async {
    var response = await sqlLite.fetchDashSales(id);
    setState(() {
      data1 = response;
      print(data1[0]["jBoar"]);
    });
...
class _GradientAppBarFb1State extends State<GradientAppBarFb1> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      title: const Text("Daily Stock Inventory", style: TextStyle(color: Colors.white)),
